...
public class YishuCollectionController {
    ...
    private static final String TABLE_NAME = "yishuCollection";
    ...
    */
    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(yishuCollection !=null){
            ...
            if(yishu != null){
            BeanUtils.copyProperties( yishu , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "username", "password", "newMoney", "yonghuId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
    ...
    */
    ...
    public R save(@RequestBody YishuCollectionEntity yishuCollection, HttpServletRequest request){
        ...
        Wrapper<YishuCollectionEntity> queryWrapper = new EntityWrapper<YishuCollectionEntity>()
            ...
            .eq("yishu_collection_types", yishuCollection.getYishuCollectionTypes())
            ;
    ...
    */
    ...
    public R update(@RequestBody YishuCollectionEntity yishuCollection, HttpServletRequest request) throws NoSuchFieldException, ClassNotFoundException, IllegalAccessException, InstantiationException {
        logger.debug("update方法:,,Controller:{},,yishuCollection:{}",this.getClass().getName(),yishuCollection.toString());
        YishuCollectionEntity oldYishuCollectionEntity = yishuCollectionService.selectById(yishuCollection.getId());//查询原先数据
...
//            yishuCollection.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
    ...
    */
    ...
    public R delete(@RequestBody Integer[] ids, HttpServletRequest request){
        ...
        List<YishuCollectionEntity> oldYishuCollectionList =yishuCollectionService.selectBatchIds(Arrays.asList(ids));//要删除的数据
...
//                            yishuCollectionEntity.setYishuCollectionTypes(Integer.valueOf(data.get(0)));   //类型 要改的
        ...
        YishuCollectionEntity yishuCollection = yishuCollectionService.selectById(id);
            if(yishuCollection !=null){
                ...
                //级联表
                    YishuEntity yishu = yishuService.selectById(yishuCollection.getYishuId());
                if(yishu != null){
                    BeanUtils.copyProperties( yishu , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "username", "password", "newMoney", "yonghuId"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setYishuId(yishu.getId());
                }
                //级联表
                    YonghuEntity yonghu = yonghuService.selectById(yishuCollection.getYonghuId());
    ...
    */
    ...
    public R add(@RequestBody YishuCollectionEntity yishuCollection, HttpServletRequest request){
        ...
        Wrapper<YishuCollectionEntity> queryWrapper = new EntityWrapper<YishuCollectionEntity>()
            ...
            .eq("yonghu_id", yishuCollection.getYonghuId())
            .eq("yishu_collection_types", yishuCollection.getYishuCollectionTypes())
//            .notIn("yishu_collection_types", new Integer[]{102})
            ;
        logger.info("sql语句:"+queryWrapper.getSqlSegment());
        YishuCollectionEntity yishuCollectionEntity = yishuCollectionService.selectOne(queryWrapper);
        if(yishuCollectionEntity==null){
